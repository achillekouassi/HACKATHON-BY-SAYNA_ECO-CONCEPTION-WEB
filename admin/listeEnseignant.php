...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['action']) && $_POST['action'] === 'insert') {
        ...
        try {
            ...
            if ($existingUser) {
                echo "Ce professeur existe déjà.";
            } else {
                ...
                echo "<script>showSuccessMessageAndRedirect('Compte enseignant Créé avec succès.', 'listeEnseignant.php', 3000);</script>";
            }
...
try {
    $stmt = $pdo->query("SELECT * FROM users WHERE role = 'teacher' AND status = 'active'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['action']) && $_POST['action'] === 'update') {
        ...
        try {
            ...
            echo "<script>alert('Utilisateur modifié avec succès.'); window.location.href = 'listeEnseignant.php';</script>";
        } catch (PDOException $e) {
            echo "Erreur de connexion : " . $e->getMessage();
        }
    }
				...
				<tbody>
                    <?php foreach ($users as $user): ?>
                        <tr>
							...
							<td><?= $user['email']; ?></td>
<td>
    <a href="#" class="btn-view" data-user-id="<?= $user['id']; ?>" data-username="<?= $user['username']; ?>" data-prenom="<?= $user['prenom']; ?>" data-telephone="<?= $user['telephone']; ?>" data-email="<?= $user['email']; ?>"><i class='bx bxs-show icon-large'></i> </a>
    <a href="?action=delete&userId=<?= $user['id']; ?>" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet enseignant ?')"><i class='bx bxs-trash icon-large'></i> </a>
</td>
</tr>
<?php endforeach; ?>
</tbody>
...
<h5 class="modal-title" id="exampleModalLabel">Nouvel Enseignant</h5>
